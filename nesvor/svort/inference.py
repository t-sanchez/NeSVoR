...
from .. import __checkpoint_dir, __svort_url, __svort_checkpoint_name
...
def reconstruct_from_stacks(transforms, stacks, res_s, s_thick, res_r):
    device = stacks[0].device
    ...
    n_stack_recon = min(3, len(stacks_pad))
...
def run_svort(dataset, model, svort, vvr, force_vvr):
    ...
    if vvr:
        ...
        volume_vvr = reconstruct_from_stacks(
            transforms_vvr, stacks_ori, res_s, s_thick, res_r
        )
    ...
    if svort or vvr:
        print(score_svort, score_vvr)
        if score_svort < score_vvr or force_vvr:
            print("use stack transformation")
            transforms_out = transforms_vvr
        else:
            print("use slice transformation")
...
def _run_svort(dataset, model, device, vvr_only):
    ...
    res_s = 1.0
    s_thick = np.mean([data.thickness for data in dataset])
    ...
    for data in dataset:
        ...
        while s[:, j2].sum() == 0:
            j2 -= 1
        # if (i2 - i1) > 128 or (j2 - j1) > 128:
        #    print('large: %s' % dir_path)
        # assert (i2 - i1) < 128 and (j2 - j1) < 128
        pad_margin = 64
        slices = F.pad(
            slices, (pad_margin, pad_margin, pad_margin, pad_margin), "constant", 0
        )
    ...
    with torch.no_grad():
        for j in range(len(stacks) - 2):
            ...
            data = {
                ...
                "volume_shape": volume_shape,  # (125, 169, 145),
                "transforms": RigidTransform.cat(
                    [transforms[0], transforms[1], transforms[j + 2]]
                ).matrix(),
                "stacks": torch.cat([stacks[0], stacks[1], stacks[j + 2]], 0),
                "positions": torch.cat(
                    [
                        torch.stack(
                            (positions[i], torch.ones_like(positions[i]) * k), -1
                        )
                        for k, i in enumerate([0, 1, j + 2])
                    ],
                    0,
                ),
            }
        ...
        ncc_weight = []
        for j in range(len(stacks)):
            slice_mask = stacks[j] > 0
            transform_diff_mean, _ = get_transform_diff_mean(
                transforms_out[j], transforms[j]
            )
            transform_stack = transform_diff_mean.compose(transforms[j])
            s_old = slice_acquisition(
                mat_update_resolution(transforms_out[j].matrix(), 1, res_r),
                volume,
                None,
                slice_mask,
                data["psf_rec"],
            ...
            s_new = slice_acquisition(
                mat_update_resolution(transform_stack.matrix(), 1, res_r),
                volume,
                None,
                slice_mask,
                data["psf_rec"],
            ...
            ncc_weight.append(slice_mask.sum((1, 2, 3)))
            ncc_old = ncc_loss(s_old, stacks[j], slice_mask, win=None, reduction="none")
            ncc_new = ncc_loss(s_new, stacks[j], slice_mask, win=None, reduction="none")
            ncc_all.append(torch.min(ncc_old, ncc_new))
            t_out = torch.where(
                (ncc_old <= ncc_new).reshape(-1, 1, 1),
                transforms_out[j].matrix(),
                transform_stack.matrix(),
            )
            transforms_out[j] = RigidTransform(t_out)
        ncc_all = torch.cat(ncc_all)
        ncc_weight = torch.cat(ncc_weight)
        score_svort = compute_score(ncc_all, ncc_weight)
        ...
        for j in range(len(stacks)):
            ...
            transform_full_new = transform_diff_mean.compose(transforms_full[j])
            transform_out = transform_full_new.matrix().clone()
            transform_out[crop_idx[j]] = transform_diff.compose(
                transforms_full[j][crop_idx[j]]
            ).matrix()
            transforms_out[j] = RigidTransform(transform_out)
            transforms_full[j] = transform_full_new

        # stack registration
        transforms_stack = []
        transforms_stack_ori = []
        params = {"res_s": res_s, "s_thick": s_thick}
        vvr = VVR(
            num_levels=3,
            num_steps=4,
        ...
        for j in range(len(stacks)):
            transforms_stack_ori.append(
                RigidTransform(transforms_ori[j].axisangle().mean(0, keepdim=True))
            )
            t1 = RigidTransform(transforms_full[j].axisangle().mean(0, keepdim=True))
            if j > 0:
                trans_first = False
                t1, ncc1 = vvr(
                    t1.axisangle(trans_first=trans_first),
                    stacks_ori[j].squeeze(1)[None, None],
                    stacks_ori[0].squeeze(1)[None, None],
                    params,
                    transforms_stack[0],
                    trans_first,
                )
                t2 = (
                    transforms_stack[0]
                    .compose(transforms_stack_ori[0].inv())
                    .compose(transforms_stack_ori[j])
                )
                t2, ncc2 = vvr(
                    t2.axisangle(trans_first=trans_first),
                    stacks_ori[j].squeeze(1)[None, None],
                    stacks_ori[0].squeeze(1)[None, None],
                    params,
                    transforms_stack[0],
                ...
                if ncc1 < ncc2 and not vvr_only:
                    transforms_stack.append(RigidTransform(t1, trans_first=trans_first))
                else:
                    transforms_stack.append(RigidTransform(t2, trans_first=trans_first))
            else:
                transforms_stack.append(t1)

        transforms_svr = []
        for j in range(len(stacks)):
            n_slice = stacks_ori[j].shape[0]
            t = torch.zeros((n_slice, 6), dtype=torch.float32, device=device)
            t[:, -1] = (
                torch.arange(n_slice, dtype=torch.float32, device=device)
                - (n_slice - 1) / 2
            ) * s_thick
            t = transforms_stack[j].compose(RigidTransform(t))
            transforms_svr.append(t)

        size_max = max([max(stacks_ori[j].shape[-2:]) for j in range(len(stacks))])
        for j in range(len(stacks)):
            if stacks_ori[j].shape[-1] < size_max or stacks_ori[j].shape[-2] < size_max:
                dx1 = (size_max - stacks_ori[j].shape[-1]) // 2
                dx2 = (size_max - stacks_ori[j].shape[-1]) - dx1
                dy1 = (size_max - stacks_ori[j].shape[-2]) // 2
                dy2 = (size_max - stacks_ori[j].shape[-2]) - dy1
                stacks_ori[j] = F.pad(stacks_ori[j], (dx1, dx2, dy1, dy2))

        # volume from svr stack
        params = {
            "psf": data["psf_rec"],
            "slice_shape": stacks_ori[0].shape[-2:],
            "interp_psf": False,
            "res_s": res_s,
            "res_r": res_r,
            "s_thick": s_thick,
        ...
        mat = mat_update_resolution(
            RigidTransform.cat(
                [transforms_svr[j][crop_idx[j]] for j in range(3)]
            ).matrix(),
            1,
            res_r,
        )
        ss = torch.cat([stacks_ori[j][crop_idx[j]] for j in range(3)])
        mask_ss = ss > 0
        volume_svr = PSFreconstruction(mat, ss, None, None, params)
        srr = SRR(n_iter=1, use_CG=True)
        volume_svr = srr(mat, ss, volume_svr, params, slices_mask=mask_ss)

        ncc_all = []
        ncc_weight = []
        for j in range(len(stacks)):
            slices_ori = stacks_ori[j][crop_idx[j]]
            slice_mask = slices_ori > 0
            s_sim = slice_acquisition(
                mat_update_resolution(
                    transforms_svr[j][crop_idx[j]].matrix(), 1, res_r
                ),
                volume_svr,
                None,
                slice_mask,
            ...
            ncc_weight.append(slice_mask.sum((1, 2, 3)))
            ncc_all.append(
                ncc_loss(s_sim, slices_ori, slice_mask, win=None, reduction="none")
            )
        ncc_all = torch.cat(ncc_all)
        ncc_weight = torch.cat(ncc_weight)
        score_svr = compute_score(ncc_all, ncc_weight)
        print(score_svort, score_svr)

        if score_svort < score_svr or vvr_only:
            print("use stack transformation")
            transforms_out = transforms_svr
        else:
            print("use slice transformation")
...
def svort_predict(
    ...
    model.eval()
    """# old model
    # model = InitializerNet(n_iter=3)
    name = "svort_iqa"
    # load checkpoint
    import os

    cp = torch.load(
        os.path.join("/home/junshen/SVoRT/results", name, "checkpoint.pt"),
        map_location=device,
    )
    model.to(device)
    model.load_state_dict(cp["model"])
    model.eval()
    """
    # return _run_svort(dataset, model, device, force_vvr)
